                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {portfolioData.projects.map((project, index) => (
                        <Motion.div
                            ...
                            className="group bg-slate-50 dark:bg-slate-800 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all border border-slate-100 dark:border-slate-700 flex flex-col will-change-transform"
                        >
                            <div className="p-8 flex-1 flex flex-col">
                                <div className="flex justify-between items-start mb-4">
                                    <h3 className="font-display text-2xl font-bold text-slate-900 dark:text-white group-hover:text-primary transition-colors">
                                        {l10n(project.title, lang)}
                                    </h3>
                                ...
                                <div className="mb-6">
                                    ...
                                    <ul className="space-y-2">
                                        {(l10n(project.features, lang) || []).map((feature, idx) => (
                                            <li key={idx} className="flex items-start text-sm text-slate-600 dark:text-slate-400">
                                                <span className="mr-2 mt-1 text-secondary">&bull;</span>
                                                {feature}
                                            </li>
                                        ))}
                                    </ul>
